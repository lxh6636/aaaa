...
//#include <winsock.h> windows下的头文件
...
void addsig(int sig, void(handler)(int)){       
    ...
    memset(&sigact, '\0', sizeof(sigact));      // bezero 清空 ，bzero(&sigact, sizeof(sigact));
...
// 向管道写数据的信号捕捉回调函数(根据 send 函数反馈的 errno 来判断 socket 的读端是否已经关闭)
void sig_to_pipe(int sig){
    int save_errno = errno;
    int msg = sig;
	//ssize_t send(int sockfd, const void *buf, size_t len, int flags);
	//设置flags为0时，send和wirte是同等的
	//把pipe写端的数据拷贝进socket的内核发送buffer中，即msg
...
int main(int argc, char* argv[]){
    ...
    if(argc <= 1){      // 形参个数，第一个为执行命令的名称，即只含有自身路径(./main)
	...
	//如果其值为假，那么它先向stderr打印一条出错信息，然后通过调用 abort 来终止程序运行
    ...
    addfd(epoll_fd, listen_fd, false, false);  // 监听文件描述符不需要 ONESHOT 且是 LT 模式
    
	// socketpair(int domain, int type, int protocol, int sv[2])
    // 创建管道(创建一对无名的、相互连接的套接子，分别为pipefd[0],pipefd[1])
    ret = socketpair(PF_UNIX, SOCK_STREAM, 0, pipefd); // PF_UNIX:指定协议，在本机通信
    assert( ret != -1 );
    set_nonblocking( pipefd[1] );               // 写管道非阻塞，写时读管道堵塞等待
    ...
    alarm(TIMESLOT);        // 让信号SIGALRM在经过TIMESLOT秒后，再传送给目前main()所在进程

	// 服务器关闭后，停止操作
    while(!stop_server){
        // 检测事件
        int num = epoll_wait(epoll_fd, events, MAX_EVENT_SIZE, -1);     // 阻塞，返回就绪事件数量
		//中断了的系统调用是没有完成的调用，它的失败是临时性的，如果再次调用则可能成功
        if(num < 0 && errno != EINTR){  // 不是信号处理程序中断导致的错误
        ...
        for(int i = 0; i < num; ++i){
            ...
            else if(sock_fd == pipefd[0] && (events[i].events & EPOLLIN)){  // events=EPOLLIN
                int sig;
                char signals[1024];
			...
			// EPOLLHUP:对应的文件描述符被挂断(如本端调用shutdown)
			// EPOLLRDHUP:对端正常关闭(如对端调用close)
			// EPOLLERR:对端已经异常断开(有行动才会出现这个错误)
        ...
        // 服务器收到pipe[0]的数据，且触发SIGALRM，将timeout置为true
		if(timeout) {
            // 定时处理任务，实际上就是调用tick()函数
            http_conn::timer_lst.tick();
            // 因为一次 alarm 调用只会引起一次SIGALARM 信号，所以我们要重新定时，以不断触发SIGALARM信号。
            alarm(TIMESLOT);
