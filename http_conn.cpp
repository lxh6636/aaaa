...
void addfd(int epoll_fd, int fd, bool one_shot, bool et){
    ...
    event.data.fd = fd;
    if(et){
		//客户端直接调用close，会触犯EPOLLRDHUP事件,通过EPOLLRDHUP属性，来判断是否对端已经关闭，这样可以减少一次系统调用
...
// 注册了EPOLLONESHOT ，操作系统最多触发其注册的一个可读、可写或者异常事件，且只触发一次
...
void http_conn::init(int sock_fd, const sockaddr_in & addr){ 
    ...
    new_timer->user_data = this; // 创建定时器(自己定义的new_timer)
    time_t curr_time = time(NULL); // 设置当前时间
    new_timer->expire = curr_time + 3 * TIMESLOT; // 时间>time(NULL)+15，则超时
    this->timer = new_timer;  // 把创建的定时器给http_conn这个类
    m_timer_lst.add_timer(new_timer);  // 把定时器加入到定时器链表(按超时时间升序排序)
}
